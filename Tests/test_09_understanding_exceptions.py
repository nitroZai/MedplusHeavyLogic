...
def test_exception_flow_1():
    ...
    try:
        fruit = fruit.upper()
        result.append("one")
        fruit.lower(1) # what happens to the control flow here?
        result.append("two")
    except:
        result.append("three")
...
def function_with_except(result):
    ...
    assert ["f:enter", "f:except" ,"f:return"] == result
...
def test_finally_on_exception():
    ...
    assert ["m:beforecall", "m:aftercall", "m:else", "m:finally"] == result
